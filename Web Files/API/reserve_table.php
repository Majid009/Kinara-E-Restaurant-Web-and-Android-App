<?php
    ...
    if( isset($_POST['user_id']) && isset($_POST['table_id']) && isset($_POST['date']) && isset($_POST['time']))
	  {
				 $table_id = clean($_POST['table_id']);
				 $q = "SELECT * FROM reservations_details WHERE table_id = '$table_id'";
         $res = mysql_query($q);
 	       if(mysql_num_rows($res) == 1){
					  echo "This table is booked already !";
				 }
				 else{
 			    ...
 			    $result = mysql_query($qry);
					if($result){
						echo "Table reserverd successfully";
					}
					else{
